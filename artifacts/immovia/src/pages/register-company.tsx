...
const PLAN_ACCENT = {
  ...
  premium: "border-amber-400 ring-2 ring-amber-200",
};

const PLAN_ICON_COLOR = {
...
const PLAN_BTN = {
  basic: "bg-primary hover:bg-primary/90 text-white",
  professional: "bg-primary hover:bg-primary/90 text-white",
            ...
            {(["basic", "professional", "premium"] as PlanType[]).map((plan) => {
              ...
              return (
                <div
                  ...
                  className={`relative cursor-pointer rounded-xl border-2 p-6 flex flex-col gap-4 transition-all ${
                    isSelected ? PLAN_ACCENT[plan] : "border-border hover:border-primary/30"
                  }`}
                >
                  {plan === "professional" && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white text-[10px] font-bold uppercase tracking-wide px-3 py-0.5 rounded-full">
                      {info.tag}
                    </span>
                  )}
                  {plan === "premium" && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-500 text-white text-[10px] font-bold uppercase tracking-wide px-3 py-0.5 rounded-full">
                      {info.tag}
                    </span>
                  )}

                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${PLAN_ICON_COLOR[plan]}`}>
                    <Icon className="w-5 h-5" />
                  </div>

                  <div>
                    <h3 className="font-serif font-bold text-lg">{info.name}</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">{l.regFee}</p>
                  </div>
                  ...
                  <Button
                    type="button"
                    size="sm"
                    className={`w-full transition-all ${isSelected ? PLAN_BTN[plan] : "bg-muted text-foreground hover:bg-muted/80"}`}
